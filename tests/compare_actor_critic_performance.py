#!/usr/bin/env python3
"""
Actor-Critic学習前後の性能比較システム
学習前のpretrained_modelと学習後のActor-Criticモデルを比較
"""
import os
import sys
import torch
...
def load_pretrained_model(model_path: str, device: torch.device) -> TransformerClassifier:
    """事前学習済みモデルを読み込み"""
...
def load_actor_critic_model(model_path: str, device: torch.device) -> ActorCriticModel:
...
def test_model_performance(model, tokenizer, test_samples, max_steps, device, model_name):
    """モデルの性能をテスト"""
    print(f"\n🧪 Testing {model_name}...")
...
def compare_actor_critic_performance(pretrained_model_path: str, actor_critic_model_path: str, 
                                   test_count: int = 10, max_steps: int = 20, device: str = "auto"):
    """Actor-Critic学習前後の性能を比較"""
    print(f"🔄 Comparing Actor-Critic performance")
    print(f"  Pretrained model: {pretrained_model_path}")
    print(f"  Actor-Critic model: {actor_critic_model_path}")
    ...
    pretrained_results = test_model_performance(
        pretrained_model, tokenizer, test_samples, max_steps, device, "Pretrained Model"
    )
    
    # Actor-Criticモデルをテスト
    actor_critic_model = load_actor_critic_model(actor_critic_model_path, device)
    extracted_transformer = extract_transformer_from_actor_critic(actor_critic_model)
    actor_critic_results = test_model_performance(
        extracted_transformer, tokenizer, test_samples, max_steps, device, "Actor-Critic Model"
    )
    
    # 結果を比較
    print(f"\n📊 Performance Comparison:")
    print(f"  {'Metric':<20} {'Pretrained':<15} {'Actor-Critic':<15} {'Improvement':<15}")
    print(f"  {'-'*20} {'-'*15} {'-'*15} {'-'*15}")
    
    success_improvement = actor_critic_results['success_rate'] - pretrained_results['success_rate']
    steps_improvement = pretrained_results['avg_steps'] - actor_critic_results['avg_steps']
    
    print(f"  {'Success Rate':<20} {pretrained_results['success_rate']:<15.2%} {actor_critic_results['success_rate']:<15.2%} {success_improvement:+.2%}")
    print(f"  {'Avg Steps':<20} {pretrained_results['avg_steps']:<15.2f} {actor_critic_results['avg_steps']:<15.2f} {steps_improvement:+.2f}")
    
    # 改善の判定
    if success_improvement > 0:
        print(f"\n🎉 Actor-Critic training improved success rate by {success_improvement:.2%}")
    elif success_improvement < 0:
        print(f"\n⚠️  Actor-Critic training decreased success rate by {abs(success_improvement):.2%}")
    else:
        print(f"\n➡️  Actor-Critic training had no effect on success rate")
    
    if steps_improvement > 0:
        print(f"🎉 Actor-Critic training reduced average steps by {steps_improvement:.2f}")
    elif steps_improvement < 0:
        print(f"⚠️  Actor-Critic training increased average steps by {abs(steps_improvement):.2f}")
    else:
        print(f"➡️  Actor-Critic training had no effect on average steps")
    ...
    return {
        ...
        'improvement': {
            ...
            'avg_steps': steps_improvement
        }
    }

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compare Actor-Critic performance")
    parser.add_argument("--pretrained_model", type=str, default="../models/pretrained_model.pth", help="Path to pretrained model")
    parser.add_argument("--actor_critic_model", type=str, required=True, help="Path to Actor-Critic model")
    ...
    results = compare_actor_critic_performance(
        args.pretrained_model, args.actor_critic_model, args.count, args.max_steps, args.device
    )
    
    print(f"\n🎯 Final Comparison Results:")
    print(f"  Success Rate Improvement: {results['improvement']['success_rate']:+.2%}")
    print(f"  Average Steps Improvement: {results['improvement']['avg_steps']:+.2f}")
